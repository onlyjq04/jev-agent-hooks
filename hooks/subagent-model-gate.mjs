...
const SONNET_DENY =
  'model: "sonnet" 默认不用（效率低、易返工）。机械活改 haiku；需要判断的执行改 opus；' +
  '确有理由用 sonnet 时在 description（workflow 里是 label）写上 sonnet-ok 再提交。'
...
process.stdin.on('end', () => {
  ...
  const reason =
    ...
    `  - sonnet 默认不用，确需时在 description/label 写 sonnet-ok\n` +
    `  - 长程无人值守自主运行 → fable\n` +
...
function checkAgent(input) {
  ...
  if (m === 'sonnet' && !SONNET_OK.test(input.description || '')) return [`Agent ${SONNET_DENY}`]
  return []
}
